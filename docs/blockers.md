...
- Resolution : (A) Job runs `/app/.venv/bin/python`. (B) Job `command = ["sh","-c", "git clone --depth 1 https://github.com/${var.github_repo}.git /srv/repo && cd /srv/repo && exec .../python -m archiviste_workers.ingest --path lore/"]`; added `git` to `workers.Dockerfile`. (C) `deploy.yml` now also tags `workers:latest`. Fixed in branch `fix/OPS-005-job-clone-repo`.
- Status : RESOLVED (pending merge + redeploy + re-test).
