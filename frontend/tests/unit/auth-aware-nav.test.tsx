...
describe("AuthAwareNav — static view links (AC1)", () => {
  ...
  it("renders the Archiviste link to /", async () => {
    // AC1 (#247): chat link present for all tiers, label "Archiviste", href "/"
    await renderNav();
  ...
  it("renders the Lacunes link to /lacunes", async () => {
    // AC1 (#247): former board link renamed to Lacunes → /lacunes
    await renderNav();
  ...
  it("renders the État & métriques link to /metriques", async () => {
    // AC1 (#247): former observability link renamed → /metriques
    await renderNav();
