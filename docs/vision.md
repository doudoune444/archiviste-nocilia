...
| **OBS-002** | Endpoint `GET /v1/status` : probes santé des 3 dépendances directes gateway (Postgres / GCS / workers-joignabilité), timeouts 2 s parallèles, cache 10 s, `200` always + `503` si aucun snapshot, pas de détail d'erreur interne exposé + widget disponibilité (`#health-widget`) sur la page. | OBS-001 | facile/moyen |
