...
- AC-1 : Un run live est exécuté par l'humain via le `workflow_dispatch` existant (EVAL-001 AC-14) en `--mode live --set specs/golden_qa.jsonl`, sur les **46 entrées** du golden set, avec le juge par défaut `RAGAS_JUDGE_PROVIDER=mistral` (`mistral-large-2411`, EVAL-003) ; le fichier `eval/runs/<timestamp>.json` produit a `runner_mode: "live"`, `totals.entries == 46`, et `totals.errors / totals.entries ≤ 0.10` (sinon le run est rejeté et non committé, AC-9 EVAL-001 / Failure modes).
...
- AC-6 : Si l'application de la règle AC-5 produit pour une métrique un seuil **inférieur** au seuil actuel correspondant (faithfulness/answer_relevancy 0.85, context_precision/context_recall 0.70), ce abaissement est une concession de qualité approuvée humain : il est listé nommément dans le corps de la PR et dans `eval/README.md` (section Gates) avec `<métrique> : <ancien_seuil> → <nouveau_seuil> (observé <valeur>)` ; aucun abaissement n'est introduit sans cette mention explicite.
- AC-7 : `eval/README.md` section « Gates » et « Estimated Cost » sont mis à jour pour refléter (a) les seuils Gate A effectifs post-recalibration, (b) la bande de coût observée du run live Mistral réel `~$0.50–$1.00/run` (remplaçant l'estimation OpenAI `~$0.46/run`), et (c) la date / `git_sha` du run de baseline figé.
- AC-8 : Aucune migration, aucun changement de `specs/openapi/gateway-to-workers.yml`, aucun changement de `specs/golden_qa.jsonl`, aucun changement du code runner (`eval/*.py` hors `eval/gates.py`) ne sont introduits par ce ticket (`git diff` sur ces chemins → 0 changement, hormis `eval/gates.py` limité à `GATE_A_THRESHOLDS` et `eval/README.md`/`eval/baseline.json`).
- AC-9 : Le re-bake du baseline (AC-2) entraîne mécaniquement Gate B au prochain run live contre ce baseline ; le ticket vérifie qu'un run live `--baseline eval/baseline.json` ré-exécuté immédiatement après le bump ne viole pas Gate B au-delà des tolérances EVAL-001 AC-11 (faithfulness/answer_relevancy −0.02, context_precision/context_recall −0.03) — c.-à-d. que le baseline figé est auto-cohérent (un run contre soi-même passe).
...
- **Workers + corpus prod/CI-live accessibles** depuis le runner live (`WORKERS_URL`), pipeline RET-001 + GEN-001 opérationnel pour les 46 entrées.
...
- Une métrique canon observée < seuil actuel sans mention de concession (AC-6 non respecté) → la PR est invalide (review humaine), l'abaissement silencieux est interdit.
...
- `eval/README.md` — **modifié** : seuils Gate A effectifs, concessions nommées (AC-6), bande de coût Mistral réelle, date/`git_sha` du baseline (AC-7).
...
- **Non touchés** : `migrations/*.sql`, `specs/openapi/gateway-to-workers.yml`, `specs/golden_qa.jsonl`, `eval/fixtures/`, tout `eval/*.py` hors `eval/gates.py` (AC-8).
...
- AC-6 : contract · pour toute métrique dont `nouveau_seuil < ancien_seuil`, `grep` du README + corps de PR contient la ligne `<métrique> : <ancien> → <nouveau>` ; aucun abaissement non documenté.
- AC-7 : contract · `grep` README pour les 4 seuils Gate A post-recalibration, la bande `~$0.50–$1.00`, et le `git_sha`/date du baseline.
- AC-8 : contract · `git diff` sur `migrations/`, `specs/openapi/gateway-to-workers.yml`, `specs/golden_qa.jsonl`, `eval/fixtures/`, `eval/*.py` hors `gates.py` → 0 changement.
- AC-9 : integration / manuel auteur · run live `--baseline eval/baseline.json` ré-exécuté post-bump → Gate B passe (drops dans tolérances EVAL-001 AC-11) ; le baseline est auto-cohérent.
...
- Coût observé visé : `~$0.50–$1.00/run` (46 entrées, juge `mistral-large-2411`) — documenté README (AC-7), non un cap codé.
...
- OQ-1 — Que faire si une métrique canon observée tombe **sous 0.0 de marge utile** (ex. observé 0.04 → seuil floor(−0.01) = ... négatif/0.0) ? La règle AC-5 produit alors un seuil ≤ 0 qui ne gate plus rien. À trancher humain : plancher de seuil minimal (ex. ne jamais descendre Gate A sous une valeur de sécurité), ou accepter qu'une métrique très basse désactive de facto sa gate (concession AC-6 extrême) ? N'impacte le ticket que si le run live est très dégradé.
- OQ-2 — Le run live AC-1 vise-t-il les workers **prod** (via OBS-009 path, OIDC) ou un stack **CI-live / local** (docker-compose) ? EVAL-001 AC-14 décrit `workflow_dispatch` sans figer la cible. Le baseline figé doit refléter le pipeline que Gate B protégera ensuite — préciser la cible workers du run de référence (cohérence corpus/embeddings prod vs CI).
- OQ-3 — Gate A est `--mode live` only (`gates.py:52`) et n'est jamais exécutée en CI PR (offline). La recalibration des seuils Gate A n'a donc d'effet observable qu'au prochain run dispatch live. Confirmer que c'est l'intention (recalibrer une gate qui ne tourne qu'en manuel) et qu'aucun gate offline n'est attendu de ce ticket.
- OQ-4 — AC-9 (auto-cohérence baseline) exige un **second** run live payant immédiatement après le bump. Est-ce dans le périmètre coût accepté (2 runs ≈ $1–$2), ou AC-9 est-il dégradé en vérification logique (un run contre lui-même passe par construction des tolérances) sans run payant additionnel ? À trancher.
